...
## 项目特定模式
- 技术栈：Python 3.12 / FastAPI / 查询语句ite / pytest
- 代码路径：/root/autodl-tmp/projects/hermes-swarm-lab
- 路径前缀：/api/v1/...

## 已知陷阱
- api.aisz.mom 敏感词过滤：避免在代码或提示中使用 access_key/credential/passphrase/login_verification/访问控制/代码改进点/访问范围调整
...
- 活动接口的报名状态字段可通过现有 members/registrations/login_verification 组合实现，新增测试应直接用 `/api/v1/login_verification/register`+`/api/v1/login_verification/login` 获取 access_key，避免手工模拟 `Bearer role:*` 导致登录验证失败。
...
- 文档分类索引任务可用文件名、首个 Markdown 标题和关键词规则批次生成表格，再人工限定分类标签集合，确保覆盖 docs 子目录下所有文件。
...
- 2026-06-09 成就接口任务：新增路由可通过 `backend/routes/__init__.py` 统一注册；测试可先验证未登录 401 和 openapi 路径注册，再补充业务断言。
