...
def make_auth_headers(client, phone, role='member'):
    ...
    import backend.repository as _repo
    import backend.routes.common as _common

    # Check if user already exists
    with _db.connect() as conn:
        existing = conn.execute('SELECT id, role FROM members WHERE phone = ?', (phone,)).fetchone()
    if existing is None:
        resp = client.post('/api/v1/members/register', json={
    ...
    member = _repo.get_member_by_phone(phone)
    assert member is not None
    return {'Authorization': f'Bearer {_common.token_for_member(member)}'}
