...
def test_worker_routing_recommendations_returns_structured_data(tmp_path: Path) -> None:
    """轻量级路由提醒接口应返回结构化数据。"""
    _init_test_db(tmp_path)
    create_task_queue_worker('backend-dev', '后端开发工程师', 'active', ['FastAPI', 'SQLite', 'backend'])
    create_task_queue_worker('reviewer-1', '代码审查员', 'active', ['review', 'quality'])
    ...
    with TestClient(app) as client:
        response = client.get(
            ...
            params={'workspace_path': str(tmp_path), 'status': 'ready'},
        )
        invalid = client.get(
            '/api/v1/workers/recommendations',
            params={'workspace_path': str(tmp_path), 'status': 'invalid'},
    ...
    assert payload['data']['status_filter'] == 'ready'
    ...
    assert payload['data']['recommendations'][0]['worker_key'] in {'backend-dev', 'reviewer-1'}
    assert invalid.status_code == 422
    assert invalid.json() == {'detail': '状态筛选参数无效'}
